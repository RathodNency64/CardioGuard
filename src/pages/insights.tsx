import { useGetModelInsights, getGetModelInsightsQueryKey } from '@workspace/api-client-react';
...
export default function Insights() {
  const query = useGetModelInsights({ query: { queryKey: getGetModelInsightsQueryKey(), staleTime: 60_000 } });
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px] px-5 py-14 sm:px-8 lg:py-20">
      ...
      {query.isLoading && <div className="mt-12 grid gap-5 sm:grid-cols-2"><div className="h-48 animate-pulse rounded-2xl bg-muted" /><div className="h-48 animate-pulse rounded-2xl bg-muted" /><div className="h-72 animate-pulse rounded-2xl bg-muted sm:col-span-2" /></div>}
      {query.isError && <div role="alert" data-testid="status-insights-error" className="mt-12 rounded-2xl border border-destructive/30 bg-destructive/5 p-6 text-destructive"><AlertCircle size={20} /><p className="mt-3 font-extrabold">Project notes are unavailable right now.</p><button type="button" onClick={() => query.refetch()} data-testid="button-retry-insights" className="mt-4 rounded-full border border-destructive/30 px-4 py-2 text-xs font-bold">Try again</button></div>}
